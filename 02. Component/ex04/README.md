...
teacher 왈
프로그래밍은 변수를 변화시켜가는 과정이다.

### 01. 기본개념
1.  state
   - 컴포넌트 내부의 현재 상태(변화되는 것. 변수 사용. 화면 변화에 사용)를 나타내는 쓰기 가능한 데이터
     - ex02의 TitleBar01 this.state
   - 컴포넌트는 this.state 안에 여러 데이터를 가질 수 있다.(class component)
   - this.state은 특정 컴포넌트 전용이며(다른 놈한테 줄 수 없음. 프로퍼티는 줄 수 있다.) 변경을 위해서는 setState() 함수를 사용한다.(class component, ex03 TitleBar01 참고)
   - 상태가 업데이트 되면 컴포넌트의 반응형(Reactive - 상태변화에 반응하는 것) Rendering이 트리거가 되고 컴포넌트와 자식 컴포넌트가 다시 렌더링 된다.(상태가 업데이트 되면, native Dom은 그대로 두고 React Dom은 처음부터 새로 그림. 부담이 적다.)
...
3.  폼 컴포넌트를 제어 컴포넌트로 만드는 것은 조금 복잡해 보이지만 다음과 같은 장점이 있다.
   ...
   - 사용자 입력 값에 대한 Validation(유효성 검사)을 할 수 있다.
4. 예제
   - src/02 제어 컴포넌트
   - src/03 비제어 컴포넌트
